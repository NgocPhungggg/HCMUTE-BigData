...
B = foreach A generate ip_c, data_bytes_s as upload;
C = group B by ip_c;
D = foreach C generate group as ip, SUM(B.upload) as sum_upload;
E = order D by sum_upload DESC;
F = limit E 100;
G = join F by ip, B by ip_c;
H = group G by ip;
I = foreach H generate group, MAX(G.upload), (double)100*MAX(G.upload)/MAX(G.sum_upload);
dump I;

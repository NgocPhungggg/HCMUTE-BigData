...
B = FOREACH A GENERATE ip_c, data_bytes_s;
C = GROUP B BY ip_c;
D = FOREACH C GENERATE group as IP, SUM(B.data_bytes_s) as upload;
E = FOREACH (GROUP D ALL) GENERATE TOP(100,1,D) AS top_users;
F = FOREACH E GENERATE FLATTEN(top_users);
G = ORDER F BY upload DESC;
Dump G;
